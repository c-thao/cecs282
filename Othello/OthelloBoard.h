...
/*
An OthelloBoard consists of all data representative of a single game of Othello.
This includes the state of the board, tracking the current player, keeping a his-
tory of moves on the board, and keeping score.
*/
class OthelloBoard {
...
public:
	...
	/*
	Default destructor cleans up any OthelloMove objects left in the vector
	mHistory
	*/
	~OthelloBoard() {
		if (!mHistory.empty()) {
			for (std::vector<OthelloMove *>::iterator i = mHistory.begin(); i != mHistory.end(); i++) {
				delete *i;
			}
			mHistory.clear();
		}
	...
	*/
	...
	/*
	Returns true if the 2 passes occur indicating the game
	is over as neither players are able to make a move
	*/
